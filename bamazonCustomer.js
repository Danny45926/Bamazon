...
//Prompts the user select desired product, fulfills the order, and then calls the new order function
function placeOrder(){
  inquirer.prompt([{
    name: 'selectId',
  ...
  connection.query('SELECT * FROM products WHERE id = ?', [answer.selectId], function(err, res){
    if(answer.selectQuantity > res[0].StockQuantity){
      ...
      console.log('We will notify you when inventory is restocked');
      console.log('');
      newOrder();
    }
    else{
...
//Lets user place another order, or close the app
function newOrder(){
  inquirer.prompt([{
    type: 'confirm',
    name: 'choice',
